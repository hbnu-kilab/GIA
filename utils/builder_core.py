...
def make_grounding(facts: Any) -> Dict[str, Any]:
    ...
    for asn, devices in core._as_groups().items():
        as_groups[str(asn)] = {
            ...
            "ibgp_missing_pairs_count": len(pre.get("bgp_missing_pairs_by_as", {}).get(asn, [])),
            "ibgp_under_peered_count": len(pre.get("bgp_under_by_as", {}).get(asn, [])),
        }
    if as_groups:
    ...
    anomalies = {
        ...
        "ssh_missing_devices": sorted(list(pre.get("ssh_missing", [])))
    }

    out = {
        "inventory": {k: v for k, v in inventory.items() if v}, # 비어있지 않은 항목만 포함
        "anomalies": {k: v for k, v in anomalies.items() if v} # 비어있지 않은 항목만 포함
    }
    return out
    """
    LLM에 제공할 '근거(grounding)' 스냅샷을 생성한다.
    AS/SSH/VRF/L2VPN 주요 집계치와 함께, _precompute에서 계산된
    네트워크 이상 징후(anomaly) 정보를 요약하여 포함한다.
    """
    fx = facts.get("devices") if isinstance(facts, dict) else facts
    if not fx:
        return {"as_groups": [], "ssh_missing_count": 0, "anomalies": {}}
...
def execute_intent(intent: Dict[str, Any], facts: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[WARNING] metric computation failed for {metric}: {e}")
